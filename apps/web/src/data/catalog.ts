import type { Modality } from "@prompt-studio/core";

export type StudioMode = Exclude<Modality, "text">;
...
export const MODE_DATA: Record<StudioMode, ModeConfig> = {
  image: {
    label: "图片",
    models: ["GPT Image", "Gemini Image", "Midjourney", "FLUX", "Stable Diffusion / SDXL", "Ideogram", "Recraft"],
    presets: ["通用生成", "人像摄影", "角色设计", "三视图 / 设定图", "产品摄影", "电影剧照", "动漫 / 国漫", "建筑 / 室内", "海报 / 排版", "灰模 / 人体比例"]
  },
  video: {
    label: "视频",
    models: ["Veo", "Sora", "Kling", "Runway", "Seedance", "Hailuo", "Luma Dream Machine", "Pika", "PixVerse"],
    presets: ["通用视频", "人物动作", "电影镜头", "Image-to-Video", "首尾帧", "角色一致性", "产品广告", "短视频运镜", "多镜头分镜"]
  },
  "3d": {
    label: "3D",
    models: ["通用 Text-to-3D", "Mesh 生成", "图像转 3D", "本地 3D 工作流"],
    presets: ["角色模型", "产品模型", "场景模型", "雕塑 / 手办", "灰模"]
  },
  audio: {
    label: "音频",
    models: ["Suno", "Udio", "Stable Audio", "ElevenLabs", "Lyria", "MusicGen"],
    presets: ["音乐", "配乐", "环境音", "音效", "旁白 / TTS", "歌曲"]
  }
};
...
export const MODELS: ModelProfile[] = [
  ["GPT Image","image","通用图像生成与编辑","自然语言、编辑、构图","OpenAI"],
  ["Gemini Image","image","多模态图像创作","参考图、编辑、文本理解","Google"],
  ["Midjourney","image","风格化视觉创作","审美、概念图、摄影风格","Cloud"],
  ["FLUX","image","图像生成生态","本地/云端、LoRA、工作流","Open"],
  ["SDXL","image","本地扩散模型生态","ControlNet、LoRA、ComfyUI","Local"],
  ["Ideogram","image","设计与文字图像","海报、排版、商业视觉","Cloud"],
  ["Veo","video","视频生成","镜头、动作、声音描述","Google"],
  ["Sora","video","视频生成","场景、运动、镜头","OpenAI"],
  ["Kling","video","图生视频 / 视频生成","人物动作、运镜、首尾帧","Cloud"],
  ["Runway","video","生成与视频创作","Camera、I2V、编辑","Cloud"],
  ["Seedance","video","视频生成","动作、镜头、角色连续性","Cloud"],
  ["Hailuo","video","视频生成","人物、镜头、短视频","Cloud"],
  ["Luma Dream Machine","video","图像与视频生成","I2V、运动","Cloud"],
  ["Text-to-3D","3d","3D 资产生成","Mesh、材质、参考图","Mixed"],
  ["Image-to-3D","3d","图片转3D","单图、多视图、Mesh","Mixed"],
  ["Suno","audio","音乐生成","歌词、风格、结构","Cloud"],
  ["Udio","audio","音乐生成","歌曲、风格、续写","Cloud"],
  ["ElevenLabs","audio","语音与音频","TTS、声音、对白","Cloud"]
].map((item, index) => ({
  id: `m${index}`,
  name: item[0] as string,
  type: item[1] as StudioMode,
  desc: item[2] as string,
  abilities: (item[3] as string).split("、"),
  vendor: item[4] as string
}));
...
export const FEATURED: FeaturedAsset[] = [
...
export const WORKFLOWS: WorkflowPreset[] = [
  {name:"角色身份锁定",type:"图片",desc:"母版身份 → 45°结构 → 90°结构 → 三视图 → 质量修复",nodes:["Reference","Prompt","Generate","Review"]},
  {name:"灰模资产建立",type:"图片",desc:"多姿势参考 → 身材特征表 → 无脸灰模 → 三视图资产",nodes:["Analyze","Parameters","Graymodel","Save"]},
  {name:"图片转视频",type:"视频",desc:"首帧 → 视频 Prompt → Camera Motion → 生成 → 选片",nodes:["Image","Motion","Video","Review"]},
  {name:"角色短片",type:"视频",desc:"角色母版 → 场景图 → 分镜 → 多镜头视频 → 连续性检查",nodes:["Identity","Storyboard","Shots","QC"]},
  {name:"产品广告",type:"视频",desc:"产品参考 → Hero Frame → 运镜 → 广告视频 → 输出",nodes:["Product","Image","Camera","Video"]},
  {name:"ComfyUI 外部工作流",type:"混合",desc:"Prompt Asset → workflow.json → Queue → 保存输出案例",nodes:["Prompt","ComfyUI","Output","Asset"]}
];
